...
class App:
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="📦 番茄钟工具", font=("Arial", 18, "bold"),
        ...
        # 根据工具名称添加特定功能
        if "timer" in "pomodoro-timer".lower() or "clock" in "pomodoro-timer".lower():
            self.setup_timer_ui(main)
        elif "random" in "pomodoro-timer".lower() or "dice" in "pomodoro-timer".lower() or "coin" in "pomodoro-timer".lower():
            self.setup_random_ui(main)
        elif "note" in "pomodoro-timer".lower() or "todo" in "pomodoro-timer".lower():
    ...
    def generate_random(self):
        if "dice" in "pomodoro-timer".lower():
            result = random.randint(1, 6)
            self.result_label.config(text=f"🎲 {result}")
        elif "coin" in "pomodoro-timer".lower():
